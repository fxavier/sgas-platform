...
// Define the InstitutionData type to match the shape in INSTITUTIONS
type InstitutionData = (typeof INSTITUTIONS)[0];
...
export function DashboardContent({
  ...
  const institutionProjects = projectsData.filter(
    (p) => p.institution === tenantId
  );
  const institutionDocuments = documentsData.filter(
    (d) => d.institution === tenantId
  );

  // Mock KPI data
  const kpiData = {
    activeProjects: institutionProjects.filter(
      (p) => p.status === 'in-progress'
    ).length,
    completedProjects: institutionProjects.filter(
    ...
    <div className='space-y-8 animate-in fade-in duration-500'>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
        <ProjectProgressChart
          data={chartData.projectProgress.filter((p) =>
